...
# Apply Median Filtering to all features
window_size_median = 3  # Window size for median filtering
median_filtered_cols = {}

for feature in df.columns:
    if feature != 'Attack_type':  # Exclude the label column
        median_filtered_cols[feature + '_median_filtered'] = df[feature].rolling(window=window_size_median, center=True).median()

# Add all new median filtered columns at once
df = pd.concat([df, pd.DataFrame(median_filtered_cols)], axis=1)
...
for feature in df.columns:
    if feature != 'Attack_type':  # Exclude the label column
        ...
        upper_bound = mean + threshold_std * std
        lower_bound = mean - threshold_std * std
        filtered_columns[feature + '_std_filtered'] = df[feature].apply(
            lambda x: x if (lower_bound <= x <= upper_bound) else None
        )

# Concatenate all new columns at once to avoid fragmentation
df = pd.concat([df, pd.DataFrame(filtered_columns)], axis=1)
